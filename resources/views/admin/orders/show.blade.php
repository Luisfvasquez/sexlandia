    ...
    <div class="space-y-6" x-data="{ showUploadModal: false }">
        ...
        {{ in_array($order->status, ['completed', 'delivered'])
            ...
            : ($order->status == 'cancelled'
                ...
                : ($order->status == 'in_transit' ? 'bg-amber-100 text-amber-700' : 'bg-yellow-100 text-yellow-700')) }}">
                    ...
                    <div>
                        ...
                        @if($order->deliveryUser)
                            <div class="mt-3 pt-3 border-t border-gray-100 text-xs">
                                <span class="font-bold text-gray-700 block">🚚 Repartidor Asignado:</span>
                                <p class="text-indigo-600 font-semibold">{{ $order->deliveryUser->name }} ({{ $order->deliveryUser->email }})</p>
                                @if($order->delivery_assigned_at)
                                    <p class="text-gray-500">Tomado: {{ $order->delivery_assigned_at->format('d/m/Y h:i A') }}</p>
                                @endif
                                @if($order->delivered_at)
                                    <p class="text-emerald-600 font-semibold mt-1">✓ Entregado: {{ $order->delivered_at->format('d/m/Y h:i A') }}</p>
                                @endif
                                @if($order->delivery_notes)
                                    <p class="text-gray-600 italic mt-1">Notas: "{{ $order->delivery_notes }}"</p>
                                @endif
                            </div>
                        @else
                            <p class="text-xs text-amber-600 mt-2 font-medium">⏳ Esperando asignación de repartidor</p>
                        @endif
                    </div>
                </div>
                            ...
                            <div class="flex flex-col gap-3">
                                ...
                                @if ($order->order_type !== 'store' && $order->status !== $nextStatus && $order->payment_status === 'pending')
                                    <button type="submit" name="status" value="{{ $nextStatus }}"
                                        ...
                                        class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-4 rounded-xl shadow transition-colors flex justify-center items-center gap-2">
                        ...
                        @forelse($order->paymentProofs as $proof)
                            <div class="border border-gray-200 rounded-xl p-4 bg-gray-50 relative">
                                ...
                                @if ($proof->images->count() > 0)
                                    ...
                                    <p class="text-center text-xs text-gray-400 mt-2">Haz clic en la imagen para ampliar
                                    </p>
                                @else
