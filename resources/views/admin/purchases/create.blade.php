...
@section('content')
    <div x-data="purchaseForm()">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-cream">Nueva Compra de Mercancía</h1>
            <span class="bg-chocolate text-white px-4 py-2 rounded-lg font-bold">
                Tasa Activa: Bs. <span x-text="exchangeRate"></span>
            </span>
        </div>

        @if ($errors->any())
            <div class="mb-4 bg-rose-500/10 border-l-4 border-rose-500 p-4 rounded-lg">
                <ul class="list-disc list-inside text-rose-300 text-sm">
        ...
        <form action="{{ route('admin.purchases.store') }}" method="POST" class="space-y-6" @submit="isSubmitting = true">
            ...
            <div class="bg-chocolate p-6 rounded-xl shadow grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    <label class="block text-sm font-medium text-cream/90 mb-1">Proveedor</label>
                    <div class="flex items-center space-x-4 mb-2">
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="radio" class="form-radio text-blush" name="supplier_type" value="existing"
                                :checked="!isNewSupplier" @change="isNewSupplier = false">
                            <span class="ml-2 text-sm text-cream/90">Existente</span>
                        </label>
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="radio" class="form-radio text-blush" name="supplier_type" value="new"
                                :checked="isNewSupplier" @change="isNewSupplier = true">
                            <span class="ml-2 text-sm text-cream/90">Nuevo</span>
                        </label>
                    </div>

                    <div x-show="!isNewSupplier">
                        <select name="supplier_id" class="w-full rounded-lg border-white/15 focus:ring-wine"
                    ...
                    <div x-show="isNewSupplier" class="space-y-2">
                        <input type="text" name="new_supplier_rif" placeholder="RIF (Ej: J-12345678-9)"
                            class="w-full rounded-lg border-white/15 text-sm" x-bind:required="isNewSupplier"
                        ...
                        <input type="text" name="new_supplier_name" placeholder="Nombre (Opcional)"
                            class="w-full rounded-lg border-white/15 text-sm"
                            oninput="this.value = this.value.replace(/[^a-zA-ZáéíóúÁÉÍÓÚüÜñÑ\s\-\.]/g, '')"
                            title="Solo letras y espacios">
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium text-cream/90 mb-1">Nº Factura / Recibo</label>
                    <input type="text" name="purchase_code" required class="w-full rounded-lg border-white/15"
                        oninput="this.value = this.value.replace(/[^a-zA-Z0-9\-\/]/g, '').toUpperCase()"
                        title="Solo letras, números, guiones y barras (Ej: FAC-0001)">
                </div>
                <div>
                    <label class="block text-sm font-medium text-cream/90 mb-1">Fecha de Compra</label>
                    <input type="date" name="purchased_at" value="{{ date('Y-m-d') }}" required
                        class="w-full rounded-lg border-white/15">
                </div>
                <div>
                    <label class="block text-sm font-medium text-cream/90 mb-1">Nota (Opcional)</label>
                    <input type="text" name="notes" class="w-full rounded-lg border-white/15"
            ...
            <div class="bg-chocolate p-6 rounded-xl shadow">
                <div class="flex justify-between items-center mb-4 border-b pb-2">
                    <h2 class="text-xl font-semibold text-cream/90">Detalle de Productos</h2>
                    <div class="space-x-2">
                        ...
                        <a href="{{ route('admin.products.create') }}" target="_blank"
                            class="text-sm bg-white/10 text-cream/90 px-3 py-1 rounded-lg hover:bg-white/15">
                ...
                <div x-ref="productsContainer" class="max-h-[450px] overflow-y-auto pr-2 space-y-4 mb-4 pb-32">
                    <template x-for="(row, index) in rows" :key="index">
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-3 p-4 border rounded-xl bg-white/5 items-end relative"
                            ...
                            <div class="md:col-span-3 relative" @click.away="row.showDropdown = false">
                                <label class="block text-xs font-bold text-cream/60">Producto</label>
                                <input type="text" x-model="row.searchQuery" @focus="row.showDropdown = true"
                                    ...
                                    class="w-full mt-1 rounded-lg border-white/15 text-sm focus:ring-wine focus:border-wine"
                                    required>
                                ...
                                <div x-show="row.showDropdown"
                                    class="absolute left-0 w-full mt-1 bg-chocolate rounded-lg shadow-xl border border-white/10 max-h-48 overflow-y-auto z-50"
                                    style="display: none;">
                                    <ul class="py-1 text-sm text-cream/90">
                                        <template x-for="prod in filteredProducts(row.searchQuery)" :key="prod.id">
                                            <li @click="selectProduct(index, prod)"
                                                class="cursor-pointer hover:bg-wine/10 hover:text-blush px-3 py-2 border-b border-white/10 flex justify-between items-center transition-colors duration-150">
                                                <div class="flex flex-col">
                                                    <span class="font-semibold text-cream" x-text="prod.name"></span>
                                                    <span class="text-xs text-cream/50 font-mono"
                                                        x-text="'SKU/Cód: ' + (prod.sku_barcode || 'N/A')"></span>
                                                </div>
                                                <span
                                                    class="text-[10px] font-bold text-blush bg-wine/10 px-2 py-0.5 rounded uppercase"
                                                    x-text="prod.unit_type === 'gram' ? 'Kilos' : 'Unidades'"></span>
                                            </li>
                                        </template>
                                        <template x-if="filteredProducts(row.searchQuery).length === 0">
                                            <li class="px-3 py-3 text-cream/60 text-xs text-center font-medium">No se
                            ...
                            <div class="md:col-span-2">
                                <label class="block text-xs font-bold text-cream/60">Presentación comprada</label>
                                <select :name="`items[${index}][bulk_id]`" x-model="row.bulk_id"
                                    @change="updateUnitCost(index)" class="w-full mt-1 rounded-lg border-white/15 text-sm"
                                    required>
                            ...
                            {{-- Cantidad Comprada --}}
                            <div class="md:col-span-2">
                                <label class="block text-xs font-bold text-cream/60">Cantidad (Ej: 5 Kilos)</label>
                                <input type="number" :step="row.unit_type === 'gram' ? '0.01' : '1'"
                                    ...
                                    class="w-full mt-1 rounded-lg border-white/15 text-sm" required inputmode="decimal"
                            ...
                            {{-- Costo Unitario --}}
                            <div class="md:col-span-2">
                                <label class="block text-xs font-bold text-cream/60">Costo Unit. (USD)</label>
                                <input type="number" step="0.01" min="0" :name="`items[${index}][unit_cost]`"
                                    x-model="row.unit_cost" class="w-full mt-1 rounded-lg border-white/15 text-sm"
                            ...
                            <div class="md:col-span-2">
                                <label class="block text-xs font-bold text-cream/60">Subtotal</label>
                                <div
                                    class="w-full mt-1 p-2 bg-white/10 rounded-lg text-sm font-bold text-cream/90 text-right">
                            ...
                            <div class="md:col-span-1 text-center">
                                <button type="button" @click="removeRow(index)"
                                    class="text-rose-400 hover:text-rose-300 font-bold mb-2 p-2">X</button>
                            </div>
                        </div>
                    </template>
                </div>
                ...
                <div class="flex justify-start">
                    <button type="button" @click="addRow()"
                        class="text-sm bg-wine text-white px-4 py-2 rounded-lg hover:bg-wine-dark font-bold shadow-md transition-colors duration-200">
                        + Agregar Fila
                    </button>
                </div>
            </div>
            ...
            <div class="flex justify-end">
                <div class="bg-chocolate text-white p-6 rounded-xl shadow-lg w-full md:w-1/3">
                    <div class="flex justify-between mb-2 text-emerald-400">
                    ...
                    <button type="submit" x-bind:disabled="isSubmitting"
                        ...
                        :class="isSubmitting ? 'bg-white/20 cursor-not-allowed' : 'bg-wine hover:bg-wine'"
