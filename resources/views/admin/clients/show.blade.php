        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 transition-all duration-300 hover:shadow-md">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div class="flex items-center space-x-4">
                    <div
                        class="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white text-2xl font-bold shadow-md shadow-blue-100">
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 text-sm text-gray-500">
                            <span class="font-mono bg-gray-100 px-2 py-0.5 rounded text-gray-600 font-semibold">C.I. / RIF:
                ...
                <div>
                    <a href="{{ route('admin.clients.index') }}"
                        class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-semibold rounded-xl text-gray-700 bg-white hover:bg-gray-50 transition-colors shadow-sm gap-2">
                                        ...
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td class="px-4 py-3 text-center">
                                                @if ($account->status === 'paid')
                                                    <span
                                                        class="px-2.5 py-0.5 inline-flex text-xs leading-5 font-extrabold rounded-full bg-green-100 text-green-800">Pagado</span>
                                                @elseif($account->status === 'partial')
                                                    <span
                                                        class="px-2.5 py-0.5 inline-flex text-xs leading-5 font-extrabold rounded-full bg-yellow-100 text-yellow-800">Parcial</span>
                                                @else
                                                    <span
                                                        class="px-2.5 py-0.5 inline-flex text-xs leading-5 font-extrabold rounded-full bg-red-100 text-red-800">Pendiente</span>
                                                @endif
                                            </td>
                                            <td class="px-4 py-3 text-center">
                                                ...
                                                @else
                                                    <span
                                                        class="text-xs text-gray-400 italic font-medium">Completado</span>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
                                        ...
                                        <tr class="hover:bg-gray-50 transition-colors">
                                                    ...
                                                    <div>
                                                        ...
                                                        @if ($order->order_type === 'store')
                                                            @if ($order->payment_status !== 'paid')
                                                                <span
                                                                    class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wider bg-amber-100 text-amber-800 border border-amber-200 shadow-xs">
                                                                    ⚠️ Fiado (POS)
                                                                </span>
                                                            @else
                                                                <span
                                                                    class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider bg-slate-100 text-slate-700 border border-slate-200">
                                                                    🛒 Venta Caja
                                                                </span>
                                                            @endif
                                                        @else
                                                            ...
                                                            <span
                                                                class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider bg-indigo-100 text-indigo-800 border border-indigo-200">
                                            ...
                                            <td class="px-4 py-3 text-center">
                                                ...
                                                @elseif($order->verification_status === 'verified')
                                                    <span
                                                        class="px-2.5 py-0.5 inline-flex text-xs leading-5 font-bold rounded-full bg-blue-100 text-blue-800">
                                                        Verificado
                                                    </span>
                                                @else
                                                    <span
                                                        class="px-2.5 py-0.5 inline-flex text-xs leading-5 font-bold rounded-full bg-red-100 text-red-800">
                                                        Rechazado
                                                    </span>
                                                @endif
                                            </td>
        ...
        <div x-show="openAbonoModal" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" role="dialog"
            ...
            <div class="flex items-center justify-center min-h-screen p-4">
                ...
                <div x-show="openAbonoModal" x-transition:enter="transition ease-out duration-200"
                    ...
                    <div
                        class="bg-gradient-to-r from-emerald-600 to-teal-600 px-6 py-4 flex justify-between items-center text-white">
                        <div>
                            <h3 class="text-lg font-bold">Registrar Abono</h3>
                            <p class="text-xs text-emerald-100 mt-0.5">Orden asociada: <span
                                    x-text="selectedAccount.orderNumber"
                                    class="font-mono font-bold bg-white/20 px-1.5 py-0.5 rounded ml-1"></span></p>
                        </div>
                        <button @click="openAbonoModal = false"
                            class="text-emerald-100 hover:text-white font-bold text-2xl leading-none transition-colors">&times;</button>
                    </div>
                    ...
                    <form method="POST" :action="'/admin/clients/{{ $client->id }}/abonos'">
                        ...
                        <div class="p-6 space-y-4">
                            ...
                            <div
                                class="bg-emerald-50 rounded-xl p-4 border border-emerald-100 flex justify-between items-center text-sm shadow-inner">
                                <span class="text-emerald-800 font-bold">Saldo Pendiente:</span>
                                <span class="font-mono font-extrabold text-emerald-700 text-lg"
                                    x-text="formatCurrency(selectedAccount.pendingAmount)"></span>
                            </div>

                            {{-- Monto del Abono --}}
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Monto
                                    del Abono</label>
                            ...
                            {{-- Método de Pago --}}
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Método
                                    de Pago</label>
                            ...
                            {{-- Fecha de Pago --}}
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Fecha de
                                    Pago</label>
                        ...
                        <div class="bg-gray-50 px-6 py-4 flex flex-col sm:flex-row-reverse gap-3 border-t border-gray-100">
                            <button type="submit" x-data="{ enviando: false }" @submit.window="enviando = true"
                                ...
                                :class="enviando ? 'opacity-50 cursor-not-allowed bg-emerald-400' :
                ...
                <div x-show="openOrderDetailsModal" x-transition:enter="transition ease-out duration-200"
                    ...
                    <div
                        ...
                        <div>
                            ...
                            <p class="text-xs text-indigo-100 mt-0.5">Orden asociada: <span
                                    x-text="selectedOrder.orderNumber"
                                    class="font-mono font-bold bg-white/20 px-1.5 py-0.5 rounded ml-1"></span></p>
                        </div>
                        <button @click="openOrderDetailsModal = false"
                            class="text-indigo-100 hover:text-white font-bold text-2xl leading-none transition-colors">&times;</button>
                    </div>
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        {{-- Productos Comprados --}}
                        <div>
                            <span class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Detalle de
                                Productos</span>
                        ...
                        {{-- Historial de Pagos --}}
                        <div>
                            <span class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Historial de
                                Pagos Realizados</span>

                            <template x-if="selectedOrder.payments.length === 0">
                                <div
                                    class="bg-gray-50 p-4 rounded-xl text-center text-xs text-gray-400 italic border border-gray-100">
                            ...
                            <template x-if="selectedOrder.payments.length > 0">
                                <div class="space-y-2 max-h-60 overflow-y-auto pr-1">
                                    <template x-for="(pay, idx) in selectedOrder.payments" :key="idx">
                                        <div
                                            class="bg-white p-3 rounded-xl border border-gray-100 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between hover:border-indigo-100 transition-all duration-200">
                                            <div class="space-y-1">
                                                <div class="flex items-center space-x-2">
                                                    <span
                                                        class="bg-blue-50 text-blue-700 font-extrabold px-2 py-0.5 rounded text-[10px] uppercase tracking-wider"
                                                        x-text="pay.method"></span>
                                                    <span class="text-xs text-gray-400 font-mono"
                                                        x-text="pay.date"></span>
                                                </div>
                                                <p class="text-xs text-gray-600 font-medium">
                                                    <strong>Referencia:</strong> <span class="font-mono text-gray-800"
