        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col lg:flex-row gap-6 lg:gap-8">
                ...
                <div class="flex-1 min-w-0 space-y-6">
                    ...
                    <div
                        class="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4">
                        <!-- Campo de Búsqueda -->
                        <form method="GET" action="{{ route('client.products') }}" class="relative w-full md:w-80 shrink-0">
                            @if (request('category'))
                        ...
                        <div
                            class="flex flex-nowrap md:flex-wrap gap-2 justify-start md:justify-end w-full md:w-auto overflow-x-auto pb-2 md:pb-0 whitespace-nowrap scrollbar-none snap-x">
                            <a href="{{ route('client.products', array_filter(['search' => request('search'), 'category' => 'all'])) }}"
                                class="px-3.5 py-2 rounded-xl text-xs font-bold transition-all duration-200 shrink-0 snap-start {{ request('category', 'all') == 'all' ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                                Todas
                            </a>
                            @foreach ($categories as $category)
                                <a href="{{ route('client.products', array_filter(['search' => request('search'), 'category' => $category->id])) }}"
                                    class="px-3.5 py-2 rounded-xl text-xs font-bold transition-all duration-200 shrink-0 snap-start {{ request('category') == $category->id ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                                    {{ $category->name }}
                                </a>
                            @endforeach
                        </div>
                                    ...
                                    <div class="mt-4 pt-4 border-t border-slate-50">
                                        ...
                                        @if (
                                            ...
                                            <div class="flex items-center gap-2" x-data="{ qty: {{ $product->unit_type === 'gram' ? '0.25' : '1' }} }">
                                                <div
                                                    class="flex items-center border border-slate-200 rounded-xl bg-slate-50 overflow-hidden shrink-0">
                                                    <button
                                                        @click="qty = Math.max({{ $product->unit_type === 'gram' ? '0.05' : '1' }}, qty - {{ $product->unit_type === 'gram' ? '0.05' : '1' }})"
                                                        class="w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center hover:bg-slate-200 text-slate-600 font-extrabold text-xs sm:text-sm transition-colors active:bg-slate-300">
                                                        -
                                                    </button>
                                                    <input type="number"
                                                        ...
                                                        class="w-10 sm:w-12 text-center bg-transparent border-none text-xs font-bold text-slate-800 p-0 focus:ring-0" />
                                                    <button
                                                ...
                                                <button @click="addToCart(@js($product), qty)"
                                                    class="flex-1 min-w-0 bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white font-bold py-2 px-2.5 sm:px-3 rounded-xl shadow-sm hover:shadow transition-all duration-300 inline-flex items-center justify-center gap-1.5 text-xs">
                    ...
                    @if ($products->hasPages())
                        <div class="pt-6 pb-2 flex justify-center w-full overflow-x-auto">
                            {{ $products->links() }}
                        </div>
                    @endif
                </div>
