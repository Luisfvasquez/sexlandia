<x-app-layout>
                        ...
                        <div x-show="deliveryType === 'delivery'" x-transition class="space-y-4 pt-2">
                            ...
                            <div class="space-y-2">
                                <div class="flex items-center justify-between">
                                    <label class="block text-slate-700 font-extrabold text-xs uppercase tracking-wider">
                                        📍 Ubicación en el Mapa para el Repartidor
                                    </label>
                                    <button type="button" id="btn-use-location"
                                        class="text-xs bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-bold px-3 py-1.5 rounded-lg border border-indigo-200 transition flex items-center gap-1">
                                        🎯 Usar mi Ubicación Actual (GPS)
                                    </button>
                                </div>
                                <p class="text-xs text-slate-500">Arrastra el marcador azul o haz clic en el mapa para fijar tu ubicación exacta de entrega.</p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Coordenadas por defecto (o del perfil del cliente)
            let defaultLat = parseFloat(latInput.value) || 10.4806; // Venezuela / Caracas por defecto
            ...
            // Inicializar inputs con posición base
                ...
                btnLocation.addEventListener('click', function() {
                    if (navigator.geolocation) {
                        ...
                        navigator.geolocation.getCurrentPosition(function(position) {
                            ...
                            setTimeout(() => { btnLocation.textContent = '🎯 Usar mi Ubicación Actual (GPS)'; }, 3000);
                        }, function(err) {
                            alert('No se pudo obtener la ubicación GPS. Por favor selecciónala en el mapa.');
                            btnLocation.textContent = '🎯 Usar mi Ubicación Actual (GPS)';
            ...
            // Recalcular dimensiones cuando el usuario alterna a la opción Delivery
            const deliveryRadio = document.querySelectorAll('input[name="delivery_type"]');
