...
<body class="bg-slate-50 text-slate-800 font-sans antialiased selection:bg-indigo-500 selection:text-white"
    x-data="storefrontCart()" x-cloak @keydown.window.escape="cartOpen = false">
    @php
            ...
            <div class="flex justify-between h-20">
                ...
                <div class="flex items-center gap-3">
                    <a href="/" class="flex items-center gap-2.5 group">
                        <div
                            class="w-11 h-11 bg-gradient-to-br from-indigo-500 to-indigo-600 rounded-2xl flex items-center justify-center text-white shadow-md shadow-indigo-500/10 group-hover:scale-105 transition-all duration-300">
                        ...
                        <span
                            class="font-outfit font-black text-xl tracking-tight bg-gradient-to-r from-slate-900 to-slate-700 bg-clip-text text-transparent group-hover:text-indigo-600 transition-colors">
                ...
                <div class="hidden md:flex items-center flex-1 max-w-md mx-8">
                    <div class="relative w-full">
                        ...
                        <input x-model="searchQuery" type="text" placeholder="Buscar productos, marcas..."
                            class="pl-10 pr-4 py-2.5 w-full bg-slate-100/80 border border-slate-200/60 rounded-2xl focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 text-sm transition-all duration-300 outline-none" />
                    </div>
                </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <button @click="cartOpen = true"
                        class="relative p-2.5 text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-2xl transition-all duration-300 group">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <span x-show="totalItemsCount() > 0" x-text="totalItemsCount()"
                            class="absolute -top-1 -right-1 bg-indigo-600 text-white font-extrabold text-xs w-5 h-5 rounded-full flex items-center justify-center border-2 border-white shadow-md animate-bounce">
                        </span>
                    </button>
                    ...
                    @if (Route::has('login'))
                        <div class="flex items-center gap-2">
                            @auth
                                @if (Auth::user()->role === 'client')
                                    <a href="{{ route('client.dashboard') }}"
                                        class="hidden sm:inline-flex bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold px-5 py-2.5 rounded-2xl text-xs tracking-wide shadow-md shadow-indigo-600/10 hover:shadow-indigo-600/25 transition-all duration-300 items-center gap-1.5">
                                ...
                                @else
                                    <a href="{{ route('dashboard') }}"
                                        class="hidden sm:inline-flex bg-slate-900 hover:bg-black text-white font-extrabold px-5 py-2.5 rounded-2xl text-xs tracking-wide shadow-md transition-all duration-300 items-center gap-1.5">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
                                ...
                                <form method="POST" action="{{ route('logout') }}" class="inline"
                                    onsubmit="localStorage.removeItem('client_shopping_cart')">
                                    @csrf
                                    <button type="submit"
                                        class="p-2.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-2xl transition-all duration-300"
                                        title="Cerrar Sesión">
                                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            @else
                                <a href="{{ route('login') }}"
                                    class="text-slate-600 hover:text-indigo-600 font-bold px-4 py-2.5 text-xs transition-colors">
                                    Iniciar Sesión
                                </a>

                                @if (Route::has('register'))
                                    <a href="{{ route('register') }}"
                                        class="bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold px-5 py-2.5 rounded-2xl text-xs tracking-wide shadow-md shadow-indigo-600/10 hover:shadow-indigo-600/25 transition-all duration-300">
                                        Registrarse
                                    </a>
                                @endif
                            @endauth
            ...
            <div class="flex-1 space-y-8">
                ...
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col gap-6">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            <h2 class="font-outfit font-black text-xl text-slate-800 tracking-tight">Catálogo de
                                Productos</h2>
                            <p class="text-xxs text-slate-500 mt-0.5">Explora nuestros artículos, marca tus cantidades
                                y agrégalos al carro.</p>
                        </div>
                        ...
                        <div class="relative w-full md:w-80 md:hidden">
                            ...
                            <input x-model="searchQuery" type="text" placeholder="Buscar productos..."
                                class="pl-10 pr-4 py-2.5 w-full bg-slate-50 border border-slate-200/60 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-sm transition-all outline-none" />
                        </div>
                    </div>
                    ...
                    <div class="border-t border-slate-50 pt-4">
                        <span
                            class="block text-slate-400 text-[10px] font-extrabold uppercase tracking-wider mb-2.5">Filtrar
                            por Categoría</span>
                        <div class="flex gap-2 overflow-x-auto pb-2 scrollbar-none whitespace-nowrap -mx-6 px-6">
                            <button @click="selectedCategory = 'all'"
                                ...
                                class="px-5 py-2.5 rounded-2xl text-xs font-bold transition-all duration-200 shrink-0 cursor-pointer">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                    @forelse($products as $product)
                        ...
                        <div x-show="filterProduct(@js($product))"
                            class="bg-white rounded-3xl shadow-sm border border-slate-100/80 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between overflow-hidden group">

                            <!-- Cabecera de la Tarjeta (Imagen o Marcador) -->
                            <div class="relative">
                                <div
                                    class="h-48 w-full bg-gradient-to-br from-indigo-50 to-slate-100 flex items-center justify-center relative overflow-hidden">
                                    ...
                                    <span
                                        class="absolute top-4 left-4 bg-white/90 backdrop-blur-sm text-indigo-700 font-extrabold text-[10px] px-3 py-1 rounded-xl shadow-sm border border-indigo-100/50">
                            ...
                            <div class="p-6 flex-1 flex flex-col justify-between">
                                <div class="space-y-2">
                                    <div class="flex justify-between items-start gap-2">
                                        <h4
                                            class="font-outfit font-extrabold text-slate-800 text-base leading-tight group-hover:text-indigo-600 transition-colors line-clamp-1">
                                            {{ $product->name }}
                                        </h4>
                                        @if ($product->brand)
                                            <span
                                                class="text-[9px] font-extrabold text-slate-400 bg-slate-50 border border-slate-100 px-2 py-0.5 rounded uppercase tracking-wider shrink-0">
                                                {{ $product->brand }}
                                            </span>
                                        @endif
                                    </div>
                                ...
                                <div class="mt-5 pt-4 border-t border-slate-50 space-y-4">
                                    ...
                                    <div class="flex justify-between items-baseline bg-slate-50/50 p-3 rounded-2xl">
                                        <div>
                                            <p
                                                class="text-slate-400 text-[9px] font-extrabold uppercase tracking-wider">
                                                Precio USD</p>
                                            <span class="text-lg font-black text-slate-800">
                                                ${{ number_format($product->display_price / $safeRate, 2, ',', '.') }}
                                            </span>
                                            <span
                                                class="text-slate-500 font-bold text-xxs">{{ $product->unit_label }}</span>
                                        </div>
                                    ...
                                    @if (
                                        ...
                                        <div class="flex items-center gap-2" x-data="{ qty: {{ $product->unit_type === 'gram' ? '0.25' : '1' }} }">
                                            <div
                                                class="flex items-center border border-slate-200/80 rounded-2xl bg-slate-50 overflow-hidden shrink-0">
                                                <button
                                                    @click="qty = Math.max({{ $product->unit_type === 'gram' ? '0.05' : '1' }}, qty - {{ $product->unit_type === 'gram' ? '0.05' : '1' }})"
                                                    class="px-2.5 py-2 hover:bg-slate-200 text-slate-600 font-extrabold text-xs transition-colors cursor-pointer">
                                                    -
                                                </button>
                                                <input type="number"
                                                    ...
                                                    class="w-12 text-center bg-transparent border-none text-xs font-bold text-slate-800 p-0 focus:ring-0 outline-none" />
                                                <button
                                            ...
                                            <button @click="addToCart(@js($product), qty)"
                                                class="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold py-2 px-3 rounded-2xl shadow-md shadow-indigo-600/10 hover:shadow-indigo-600/25 transition-all duration-300 inline-flex items-center justify-center gap-1.5 text-xs cursor-pointer">
                                    ...
                                    @else
                                        <button disabled
                                            class="w-full bg-slate-100 text-slate-400 font-bold py-3 rounded-2xl text-xs cursor-not-allowed">
                    ...
                    @empty
                        <div
                            class="col-span-full py-16 text-center bg-white border border-slate-100 rounded-3xl space-y-4">
            ...
            <div class="hidden lg:block w-80 space-y-6 shrink-0">
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 space-y-4">
                    ...
                    <ul class="space-y-4 text-xs font-medium text-slate-500">
                        <li class="flex gap-3">
                            <span
                                class="w-6 h-6 rounded-full bg-indigo-50 text-indigo-600 font-extrabold flex items-center justify-center shrink-0">1</span>
                            <span>Añade todos los productos que gustes al carrito.</span>
                        </li>
                        <li class="flex gap-3">
                            <span
                                class="w-6 h-6 rounded-full bg-indigo-50 text-indigo-600 font-extrabold flex items-center justify-center shrink-0">2</span>
                            <span>Ingresa tu cuenta o regístrate en minutos para identificarte.</span>
                        </li>
                        <li class="flex gap-3">
                            <span
                                class="w-6 h-6 rounded-full bg-indigo-50 text-indigo-600 font-extrabold flex items-center justify-center shrink-0">3</span>
                ...
                @guest
                    <div
                        class="bg-gradient-to-r from-indigo-600 to-indigo-700 p-6 rounded-3xl text-white space-y-4 shadow-md shadow-indigo-600/10">
                        <h3 class="font-outfit font-black text-base tracking-tight">¡Crea tu Cuenta!</h3>
                        <p class="text-xs text-indigo-100 font-medium">
                            Regístrate y obtén un panel exclusivo de cliente donde ver tus facturas pendientes, abonos,
                            pagos históricos y estatus de despachos.
                        </p>
                        <a href="{{ route('register') }}"
                            class="block bg-white hover:bg-slate-100 text-indigo-700 font-black text-center py-3 rounded-2xl text-xs transition-colors shadow-sm">
                            Registrarme Ahora
                        </a>
                    </div>
                @endguest
    ...
    <div x-show="cartOpen" @click="cartOpen = false" x-transition:enter="transition ease-out duration-300"
        x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
        x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100"
        x-transition:leave-end="opacity-0" class="fixed inset-0 bg-slate-950/40 backdrop-blur-xs z-45"></div>
    ...
    <div x-show="cartOpen" x-transition:enter="transition ease-out duration-300 transform"
        ...
        class="fixed inset-y-0 right-0 w-full sm:w-96 bg-white shadow-2xl z-50 flex flex-col justify-between overflow-hidden">
        ...
        <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            ...
            <button @click="cartOpen = false"
                class="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-xl transition-colors cursor-pointer">
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-4 scrollbar-none">
            <template x-if="cart.length === 0">
                <div class="text-center py-20 text-slate-400 flex flex-col items-center justify-center gap-4">
                    <div
                        class="w-16 h-16 bg-slate-50 border border-slate-100 rounded-full flex items-center justify-center text-slate-300">
                    ...
                    <div>
                        ...
                        <span class="text-xxs text-slate-400 mt-1 block">Añade productos de la lista a la
                            izquierda.</span>
                    </div>
                </div>
            </template>
            ...
            <template x-for="(item, index) in cart" :key="item.id">
                <div class="flex gap-4 py-3 items-center justify-between border-b border-slate-50">
                    <div class="flex-1 min-w-0">
                        ...
                        <span class="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider block mt-0.5"
                            x-text="item.category_name"></span>

                        <!-- Cantidades -->
                        <div class="flex items-center gap-2 mt-2">
                            <div
                                class="flex items-center border border-slate-200 rounded-lg overflow-hidden bg-slate-50 shrink-0">
                                <button @click="changeQtyInCart(item.id, item.unit_type === 'gram' ? -0.05 : -1)"
                                    class="px-2 py-0.5 hover:bg-slate-200 text-slate-500 font-extrabold text-xxs transition-colors cursor-pointer">-</button>
                    ...
                    <div class="text-right flex flex-col justify-between items-end h-full min-w-[6.5rem]">
                        <button @click="removeFromCart(item.id)"
                            class="text-slate-300 hover:text-rose-600 transition-colors p-1.5 rounded-lg hover:bg-rose-50 mb-2 cursor-pointer">
                        ...
                        <div>
                            ...
                            @if ($exchangeRate)
                                <span class="text-indigo-600 font-bold text-[10px] block"
            ...
            <div class="space-y-2">
                <div
                    ...
                    <span x-text="formatCurrency(calculateTotal() / {{ $safeRate }})"
                        class="text-slate-800 font-extrabold"></span>
                </div>
                    ...
                    <div>
                        ...
                        @if ($exchangeRate)
                            <span x-text="formatCurrency(calculateTotal(), ' BS')"
                                class="text-xs text-indigo-600 font-bold block mt-0.5"></span>
                        @endif
                    </div>
                    <span x-text="formatCurrency(calculateTotal() / {{ $safeRate }})"
                        class="text-2xl font-black text-slate-800 font-outfit"></span>
                </div>
            </div>

            <!-- Botón de Orden (Invitado vs Autenticado) -->
            <button :disabled="cart.length === 0" @click="handleCheckout()"
    ...
    @auth
        @if (Auth::user()->role === 'client' && $client)
            <div x-show="checkoutOpen"
                ...
                x-transition>
                <div @click.away="checkoutOpen = false"
                    ...
                    <div class="px-6 py-5 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
                        <div class="flex items-center gap-2.5">
                            <div
                                class="w-10 h-10 bg-indigo-50 rounded-xl flex items-center justify-center border border-indigo-100 shrink-0">
                    ...
                    <form action="{{ route('client.checkout') }}" method="POST" class="p-6 space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea name="delivery_address" rows="3" required
                                ...
                                class="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 text-sm transition-all outline-none">{{ old('delivery_address', $client->address) }}</textarea>
                        </div>

                        <!-- Notas -->
                        <div class="space-y-2">
                            <label
                                class="block text-slate-700 font-extrabold text-xs uppercase tracking-wider">Observaciones
                                (Opcional)</label>
                            <textarea name="notes" rows="2" placeholder="Indicaciones para el despacho..."
                                class="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 text-sm transition-all outline-none">{{ old('notes') }}</textarea>
                        </div>
                        ...
                        <div class="flex items-center gap-3 pt-4 border-t border-slate-100">
                            <button type="button" @click="checkoutOpen = false"
                                class="w-1/3 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold py-3.5 rounded-xl text-xs transition-colors cursor-pointer">
                                Cancelar
                            </button>
                            <button type="submit"
                                class="flex-1 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white font-extrabold py-3.5 rounded-xl shadow-lg shadow-emerald-500/15 transition-all duration-300 flex items-center justify-center gap-1.5 text-xs cursor-pointer">
                                Confirmar Orden
                            </button>
                        </div>
                    </form>
    ...
    <div x-show="authPromptOpen"
        class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 transition-all duration-300"
        x-transition>
        <div @click.away="authPromptOpen = false"
            class="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl border border-slate-100 p-8 text-center space-y-6">
            <div
                class="w-16 h-16 bg-indigo-50 border border-indigo-100 rounded-full flex items-center justify-center text-indigo-600 mx-auto">
                <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="space-y-2">
                ...
                <p class="text-xs text-slate-400 font-medium">
                    Para finalizar tu orden y habilitar los detalles de despacho, por favor inicia sesión o crea una
                    cuenta. Tus productos en el carrito se mantendrán perfectamente guardados en este navegador.
                </p>
            </div>

            <div class="flex flex-col gap-3 pt-2">
                <a href="{{ route('login') }}"
                    class="bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold py-3.5 rounded-2xl text-xs transition-colors shadow-sm tracking-wide">
                    Iniciar Sesión
                </a>
                <a href="{{ route('register') }}"
                    class="bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 font-extrabold py-3.5 rounded-2xl text-xs transition-colors tracking-wide">
                    Crear Cuenta de Cliente
                </a>
                <button @click="authPromptOpen = false"
                    class="text-slate-400 hover:text-slate-600 text-xxs font-extrabold tracking-wider uppercase pt-2 cursor-pointer">
                    Seguir Explorando
                </button>
            </div>
        </div>
            ...
            return {
                ...
                handleCheckout() {
                    if (!this.isAuthenticated) {
                        ...
                        alert(
                            'Tu cuenta tiene rol administrativo. Para realizar pedidos, inicia sesión con una cuenta de cliente.'
                        );
                    }
                }
            };
